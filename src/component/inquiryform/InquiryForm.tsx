...
function InquiryForm() {
  ...
  const handleConstructionSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({
        type: res?.payload?.success ? "success" : "danger",
        text:
          res?.payload?.message ||
          (res?.payload?.success
    ...
    try {
      ...
      setMessage({
        ...
        text:
          res?.payload?.message ||
          (res?.payload?.success
            ? "Design inquiry submitted!"
            : "Design inquiry failed."),
      });
      if (res?.payload?.success) {
        setDesignForm({
          full_name: "",
    ...
    try {
      ...
      setMessage({
        ...
        text:
          res?.payload?.message ||
          (res?.payload?.success
            ? "Finishing inquiry submitted!"
            : "Finishing inquiry failed."),
      });
      if (res?.payload?.success) {
        setFinishingForm({
          name: "",
    ...
    try {
      ...
      setMessage({
        ...
        text:
          res?.payload?.message ||
          (res?.payload?.success
            ? "Sololearn inquiry submitted!"
            : "Sololearn inquiry failed."),
      });
      if (res?.payload?.success) {
        setsoloForm({
          name: "",
                      ...
                      <div className="design-type-categories">
                        <Row>
                          {Object.keys(designExamples).map((style, i) => (
                            <Col md={4} className="h-400" key={i}>
            ...
            {selectedStyle &&
              designExamples[selectedStyle].map((example) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`example-card border rounded p-2 position-relative ${
                    ...
                    >
                      ...
                      {/* Image name */}
                      <p className="mt-2 mb-0 small fw-medium">{fileName}</p>
                    </div>
                  </Col>
                );
              })}
